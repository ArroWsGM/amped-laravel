...
use Arrowsgm\Amped\Exceptions\StyleSanitizerException;
use DOMAttr;
use DOMDocument;
use DOMElement;
use DOMNode;
...
class StyleSanitizer extends AMP_Base_Sanitizer
{
    /**
     ...
     * @private
     * @since 1.1
     * @see StyleSanitizer::prepare_stylesheet()
     */
     ...
     *       checking to see if any of the related elements exist in the page in `StyleSanitizer::has_used_attributes()`.
     *       Nevertheless, selectors mentioning these attributes are very numerous, so tree-shaking improvements will be marginal.
     *
     * @see StyleSanitizer::has_used_attributes()
     *
     * @since 1.1
     * @var array
     */
    ...
    public function __construct(DOMDocument $dom, array $args = [])
    {
        ...
        $guessurl = \Illuminate\Support\Facades\URL::to('/');

        $this->base_url = untrailingslashit($guessurl);
        $this->content_url = $guessurl; //WP_CONTENT_URL;
    ...
    private function get_used_class_names()
    {
        ...
        foreach ($this->xpath->query('//*/@' . AmpDom::AMP_BIND_DATA_ATTR_PREFIX . 'class') as $bound_class_attribute) {
     ...
     * @return string|StyleSanitizerException Unrelativized path or StyleSanitizerException if there is too much relativity.
     */
    private function unrelativize_path($path)
    {
        ...
        if (preg_match('#(^|/)\.+/#', $path)) {
            ...
            return new StyleSanitizerException(sprintf('There are remaining relative path segments: %s', $path));
        }
     ...
     * @return string|StyleSanitizerException Style's absolute validated filesystem path, or StyleSanitizerException when error.
    ...
    {
        if (!is_string($url)) {
            return new StyleSanitizerException('url_not_string');
        }
        ...
        if (empty($parsed_url['host'])) {
            ...
            return new StyleSanitizerException('no_url_host');
        }
        if (empty($parsed_url['path'])) {
            /* translators: %s is the original URL */
            return new StyleSanitizerException('no_url_path');
        }

        $path = $this->unrelativize_path($parsed_url['path']);
        if ($path instanceof StyleSanitizerException) {
            return $path;
        }
        ...
        if (!empty($allowed_extensions)) {
            ...
            if (!preg_match($pattern, $url)) {
                ...
                return new StyleSanitizerException('disallowed_file_extension');
            }
        }

        $base_path = public_path();
        $file_path = null;
        if ( 0 === strpos( $url, url('storage') ) ) {
            $base_path = storage_path();
            $file_path = substr( $url, strlen( url('storage') ) - 1 );
        } else {
            $file_path = substr( $url, strlen( url('') ) - 1 );
        }

        if (!$file_path || false !== strpos($file_path, '../') || false !== strpos($file_path, '..\\')) {
            /* translators: %s: the file URL. */
            return new StyleSanitizerException('file_path_not_allowed');
        }
        if (!file_exists($base_path . $file_path)) {
            /* translators: %s: the file URL. */
            return new StyleSanitizerException('file_path_not_found');
        }
    ...
    private function process_link_element(DOMElement $element)
    {
        ...
        if (!($css_file_path instanceof StyleSanitizerException)) {
     ...
     * @return string|StyleSanitizerException Stylesheet contents or WP_Error.
    ...
    private function fetch_external_stylesheet($url)
    {
        ...
        if (null === $contents) {
            try {
                ...
                $ct = is_array($r->getHeader('content-type')) && !empty($r->getHeader('content-type')) ? $r->getHeader('content-type')[0] : '';
                if ($code < 200 || $code >= 300) {
                    ...
                    $contents = new StyleSanitizerException($code);
                } elseif (!preg_match('#^text/css#', $ct)) {
                    $contents = new StyleSanitizerException('no_css_content_type');
                } else {
                    $contents = $r->getBody()->getContents();
                }
            } catch (ClientException $e) {
                $contents = new StyleSanitizerException('http_error');
            }

            Cache::put($cache_key, $contents, self::EXP_MONTH);
        }
        return $contents;
    }
    ...
    private function process_stylesheet($stylesheet, $options = [])
    {
        ...
        $cache_impacting_options = array_merge(
            ...
            [
                'language' => config('app.locale'), // Used to tree-shake html[lang] selectors.
            ]
        );

        $cache_key = md5($stylesheet . json_encode($cache_impacting_options));

        $parsed = Cache::get($cache_group . '-' . $cache_key);
        ...
        if (!$parsed || !isset($parsed['stylesheet']) || !is_array($parsed['stylesheet'])) {
             ...
             */
            Cache::put($cache_group . '-' . $cache_key, $parsed, self::EXP_MONTH);
        }
    ...
    private function parse_import_stylesheet(Import $item, CSSList $css_list, $options)
    {
        ...
        if ($this->allowed_font_src_regex && preg_match($this->allowed_font_src_regex, $https_import_stylesheet_url)) {
            ...
            trigger_error(
                sprintf(
                    'It is not a best practice to use %1$s to load font CDN stylesheets. Please use %2$s to enqueue %3$s as its own separate script.',
                    '@import',
                    'wp_enqueue_style()',
        ...
        if ($css_file_path instanceof StyleSanitizerException && ('disallowed_file_extension' === $css_file_path->getMessage())) {
            $contents = $this->fetch_external_stylesheet($import_stylesheet_url);
            if ($contents instanceof StyleSanitizerException) {
                $error = [
            ...
            $stylesheet = $contents;
        } elseif ($css_file_path instanceof StyleSanitizerException) {
            $error = [
                'code'    => $css_file_path->getCode(),
                'message' => $css_file_path->getMessage(),
                'type'    => AMP_Validation_Error_Taxonomy::CSS_ERROR_TYPE,
                'url'     => $import_stylesheet_url,
            ];
            $sanitized = $this->should_sanitize_validation_error($error);
     ...
     * @param array $options Options. See definition in StyleSanitizer::process_stylesheet().
     * @return array {
     *    Parsed stylesheet.
     *
    ...
    private function parse_stylesheet($stylesheet_string, $options)
    {
        ...
        try {
            ...
            $css_parser = new Parser($stylesheet_string, $parser_settings);
     ...
     *
     * @see StyleSanitizer::should_sanitize_validation_error()
     * @var array
     */
    ...
    private function process_font_face_at_rule(AtRuleSet $ruleset, $options)
    {
        ...
        if (!empty($options['stylesheet_url'])) {
            ...
            $stylesheet_base_url = rtrim( $stylesheet_base_url, '/\\' ).'/';
        }
    ...
    private function finalize_styles()
    {
        ...
        if ($stylesheet_groups['custom']['included_count'] > 0) {
            ...
            if ($include_manifest_comment && !empty($included_sources) && $included_original_size > 0) {
                $comment .= sprintf(
                    /* translators: %s: style[amp-custom] */
                        'The %s element is populated with:',
                        'style[amp-custom]'
                    ) . "\n" . implode("\n", $included_sources) . "\n";
                if (self::has_required_php_css_parser()) {
                    $comment .= sprintf(
                        /* translators: 1: number of included bytes. 2: percentage of total CSS actually included after tree shaking. 3: total included size. */
                            ...
                            $included_original_size
                        ) . "\n";
                } else {
                    $comment .= sprintf(
                        /* translators: %s: number of included bytes. */
                            'Total included size: %s bytes',
                            $included_size
                        ) . "\n";
                }
            }
            if ($include_manifest_comment && !empty($excluded_sources) && $excluded_original_size > 0) {
                ...
                $comment .= sprintf(
                    /* translators: %s: style[amp-custom] */
                        'The following stylesheets are too large to be included in %s:',
                        'style[amp-custom]'
                    ) . "\n" . implode("\n", $excluded_sources) . "\n";
                ...
                if (self::has_required_php_css_parser()) {
                    $comment .= sprintf(
                        /* translators: 1: number of excluded bytes. 2: percentage of total CSS actually excluded even after tree shaking. 3: total excluded size. */
                            ...
                            $excluded_original_size
                        ) . "\n";
                } else {
                    $comment .= sprintf(
                        /* translators: %s: number of excluded bytes. */
                            'Total excluded size: %s bytes',
                            $excluded_size
                        ) . "\n";
                }
            ...
            if ($include_manifest_comment && !self::has_required_php_css_parser()) {
                $comment .= "\nWarning! AMP CSS processing is limited because a conflicting version of PHP-CSS-Parser has been loaded by another plugin or theme. Tree shaking is not available.\n";
            }
