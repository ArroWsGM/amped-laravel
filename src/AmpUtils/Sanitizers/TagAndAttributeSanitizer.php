...
use Arrowsgm\Amped\Exceptions\TagAndAttributeSanitizerException;
use DOMAttr;
use DOMDocument;
use DOMElement;
use DOMNode;
 ...
 *                              must be present.
 */
class TagAndAttributeSanitizer extends AMP_Base_Sanitizer
{
    ...
    private function process_node(DOMElement $node)
    {
        ...
        if (!empty($cdata) && $node instanceof DOMElement) {
            ...
            if ($validity instanceof TagAndAttributeSanitizerException) {
                $this->remove_node($node);
                return null;
            }
        }
        ...
        if (!empty($tag_spec['unique'])) {
            $removed = false;
            $tag_spec_key = json_encode($tag_spec);
        ...
        if ($node instanceof DOMElement && !in_array('amp-bind', $this->script_components, true)) {
            foreach ($node->attributes as $name => $value) {
                if (AmpDom::AMP_BIND_DATA_ATTR_PREFIX === substr($name, 0, 14)) {
     ...
     * @return true|TagAndAttributeSanitizerException True when valid or error when invalid.
     * @since 0.7
     *
     */
    private function validate_cdata_for_node(DOMElement $element, $cdata_spec)
    {
        if (
            ...
            return new TagAndAttributeSanitizerException('excessive_bytes');
        }
        if (isset($cdata_spec['blacklisted_cdata_regex'])) {
            if (preg_match('@' . $cdata_spec['blacklisted_cdata_regex']['regex'] . '@u', $element->textContent)) {
                return new TagAndAttributeSanitizerException($cdata_spec['blacklisted_cdata_regex']['error_message']);
            }
            ...
            if (!preg_match($delimiter . $cdata_spec['cdata_regex'] . $delimiter . 'u', $element->textContent)) {
                return new TagAndAttributeSanitizerException('cdata_regex');
            }
        }
        return true;
    }
    ...
    private function is_amp_allowed_attribute(DOMAttr $attr_node, $attr_spec_list)
    {
        ...
        if (
            ...
            ||
            ('data-' === substr($attr_name, 0, 5) && AmpDom::AMP_BIND_DATA_ATTR_PREFIX !== substr($attr_name, 0, 14))
            ||
