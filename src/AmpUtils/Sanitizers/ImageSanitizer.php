...
use Arrowsgm\Amped\AmpUtils\ImageDimensionExtractor;
use DOMElement;
use DOMNode;
use DOMNodeList;
...
class ImageSanitizer extends AMP_Base_Sanitizer
{
    ...
    private function determine_dimensions( $need_dimensions ) {
        ...
        $dimensions_by_url = ImageDimensionExtractor::extract( array_keys( $need_dimensions ) );
    ...
    private function adjust_and_replace_node( $node ) {
        ...
        $old_attributes = AmpDom::get_node_attributes_as_assoc_array( $node );
        ...
        $this->add_or_append_attribute( $new_attributes, 'class', 'amp-enforced-sizes' );
        ...
        $img_node = AmpDom::create_node( $this->dom, $new_tag, $new_attributes );
    ...
    private function maybe_add_lightbox_attributes( $attributes, $node ) {
        ...
        // With alignment, the structure changes from figure.block-image > img
        // to div.block-image > figure > img and the amp-lightbox attribute
        ...
        $parent_attributes = AmpDom::get_node_attributes_as_assoc_array( $parent_node );
    ...
    /**
     * Gets whether a node has the class 'block-image', meaning it is a wrapper for an Image block.
     *
     * @param DOMElement $node A node to evaluate.
     * @return bool Whether the node has the class 'block-image'.
     */
